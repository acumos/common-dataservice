...
public class CommonDataServiceRestClientImpl implements ICommonDataServiceRestClient {
	...
	@Override
	public RestPageResponse<MLPLicenseProfileTemplate> getLicenseTemplates(RestPageRequest pageRequest) {
		URI uri = buildUri(new String[] { CCDSConstants.LICENSE_PATH, CCDSConstants.TEMPlATE_PATH }, null, pageRequest);
		logger.debug("getLicenseTemplates: uri {}", uri);
		ResponseEntity<RestPageResponse<MLPLicenseProfileTemplate>> response = restTemplate.exchange(uri, HttpMethod.GET, null,
				new ParameterizedTypeReference<RestPageResponse<MLPLicenseProfileTemplate>>() {
				});
		return response.getBody();
	}

	@Override
	public MLPLicenseProfileTemplate getLicenseTemplate(long licenseId) {
		URI uri = buildUri(
				new String[] { CCDSConstants.LICENSE_PATH, CCDSConstants.TEMPlATE_PATH, Long.toString(licenseId) },
				null, null);
		logger.debug("getLicenseTemplate: uri {}", uri);
	...
	@Override
	public MLPLicenseProfileTemplate createLicenseTemplate(MLPLicenseProfileTemplate licenseTemplate) {
		URI uri = buildUri(new String[] { CCDSConstants.LICENSE_PATH, CCDSConstants.TEMPlATE_PATH }, null, null);
		logger.debug("createLicenseTemplate: url {}", uri);
		return restTemplate.postForObject(uri, licenseTemplate, MLPLicenseProfileTemplate.class);
	}

	@Override
	public void updateLicenseTemplate(MLPLicenseProfileTemplate licenseTemplate) {
		...
		logger.debug("updateLicenseTemplate: uri {}", uri);
		restTemplate.put(uri, licenseTemplate);
	}

	@Override
	public void deleteLicenseTemplate(long licenseId) {
		URI uri = buildUri(
				new String[] { CCDSConstants.LICENSE_PATH, CCDSConstants.TEMPlATE_PATH, Long.toString(licenseId) },
				null, null);
		logger.debug("deleteLicenseTemplate: url {}", uri);
		restTemplate.delete(uri);
	}
