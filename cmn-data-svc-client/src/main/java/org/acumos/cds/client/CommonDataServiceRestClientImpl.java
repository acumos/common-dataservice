...
public class CommonDataServiceRestClientImpl implements ICommonDataServiceRestClient {
	...
	public ThreadCommentsTransport getThreadComments(String threadId) {
		...
		ResponseEntity<ThreadCommentsTransport> response = restTemplate.exchange(uri, HttpMethod.GET, null,
				new ParameterizedTypeReference<ThreadCommentsTransport>() {
				});
		return response.getBody();
	}
