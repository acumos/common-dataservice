...
public class DomainTest extends AbstractModelTest {
	...
	private void checkMLPPeer(MLPPeer m) {
		...
		Assert.assertEquals(d1, m.getCreated());
		Assert.assertEquals(s3, m.getDescription());
		Assert.assertEquals(s4, m.getLicenseStatusCode());
		...
		Assert.assertEquals(s9, m.getVulnerabilityStatusCode());
	...
	public void testMLPPeer() {
		...
		m.setStatusCode(s7);
		m.setSubjectName(s8);
		m.setVulnerabilityStatusCode(s9);
		m.setWebUrl(s10);
		checkMLPPeer(m);
		m = new MLPPeer(m);
		checkMLPPeer(m);
	...
	private void checkMLPSolutionRevision(MLPSolutionRevision m) {
		...
		Assert.assertEquals(s10, m.getLicenseStatusCode());
		Assert.assertEquals(s11, m.getVulnerabilityStatusCode());
		Assert.assertEquals(s12, m.getVersion());
	...
	public void testMLPSolutionRevision() {
		...
		m.setSourceId(s8);
		m.setUserId(s9);
		m.setLicenseStatusCode(s10);
		m.setVulnerabilityStatusCode(s11);
		m.setVersion(s12);
	...
	private void checkMLPUser(MLPUser m) {
		...
		Assert.assertEquals(d4, m.getVerifyExpiration());
	}
