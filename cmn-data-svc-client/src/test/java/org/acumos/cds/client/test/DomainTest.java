...
public class DomainTest extends AbstractModelTest {
	...
	private void checkMLPUserNbMap(MLPUserNotebookMap m) {
		Assert.assertEquals(s1, m.getUserId());
		Assert.assertEquals(s2, m.getNotebookId());
	}
	...
	public void testMLPUserNbMap() {
		MLPUserNotebookMap m = new MLPUserNotebookMap(s1, s1);
		m = new MLPUserNotebookMap(m);
		m = new MLPUserNotebookMap();
		m.setUserId(s1);
		m.setNotebookId(s2);
		checkMLPUserNbMap(m);
		m = new MLPUserNotebookMap(m);
		...
		logger.info(m.toString());
		try {
			new MLPUserNotebookMap(null, null);
			Assert.assertTrue("Unexpected success", false);
		} catch (IllegalArgumentException iae) {
			// null arg is rejected
		}
		MLPUserNotebookMap.UserNbMapPK pk = new MLPUserNotebookMap.UserNbMapPK();
		pk = new MLPUserNotebookMap.UserNbMapPK(s1, s2);
		Assert.assertFalse(pk.equals(null));
		Assert.assertFalse(pk.equals(new Object()));
		Assert.assertTrue(pk.equals(pk));
		Assert.assertFalse(pk.hashCode() == 0);
	...
	private void checkMLPUserPlMap(MLPUserPipelineMap m) {
		Assert.assertEquals(s1, m.getUserId());
		Assert.assertEquals(s2, m.getPipelineId());
	}
	...
	public void testMLPUserPlMap() {
		MLPUserPipelineMap m = new MLPUserPipelineMap(s1, s1);
		m = new MLPUserPipelineMap(m);
		m = new MLPUserPipelineMap();
		m.setUserId(s1);
		m.setPipelineId(s2);
		checkMLPUserPlMap(m);
		m = new MLPUserPipelineMap(m);
		...
		logger.info(m.toString());
		try {
			new MLPUserPipelineMap(null, null);
			Assert.assertTrue("Unexpected success", false);
		} catch (IllegalArgumentException iae) {
			// null arg is rejected
		}
		MLPUserPipelineMap.UserPlMapPK pk = new MLPUserPipelineMap.UserPlMapPK();
		pk = new MLPUserPipelineMap.UserPlMapPK(s1, s2);
		Assert.assertFalse(pk.equals(null));
		Assert.assertFalse(pk.equals(new Object()));
		Assert.assertTrue(pk.equals(pk));
		Assert.assertFalse(pk.hashCode() == 0);
	...
	private void checkMLPUserProjMap(MLPUserProjMap m) {
		Assert.assertEquals(s1, m.getUserId());
		Assert.assertEquals(s2, m.getProjectId());
	}

	@Test
	public void testMLPUserProjMap() {
		MLPUserProjMap m = new MLPUserProjMap(s1, s1);
		m = new MLPUserProjMap(m);
		m = new MLPUserProjMap();
		m.setUserId(s1);
		m.setProjectId(s2);
		checkMLPUserProjMap(m);
		m = new MLPUserProjMap(m);
		checkMLPUserProjMap(m);
		Assert.assertFalse(m.equals(null));
		Assert.assertFalse(m.equals(new Object()));
		...
		logger.info(m.toString());
		try {
			new MLPUserProjMap(null, null);
			Assert.assertTrue("Unexpected success", false);
		} catch (IllegalArgumentException iae) {
			// null arg is rejected
		}
		MLPUserProjMap.UserProjMapPK pk = new MLPUserProjMap.UserProjMapPK();
		pk = new MLPUserProjMap.UserProjMapPK(s1, s2);
		Assert.assertFalse(pk.equals(null));
		Assert.assertFalse(pk.equals(new Object()));
		Assert.assertTrue(pk.equals(pk));
		Assert.assertFalse(pk.hashCode() == 0);
