...
public class ClientMethodTest {
	...
	public void coverClientMethods() {
		...
		try {
			...
			client.findPortalSolutions(array, array, true, array, array, array, null, array, array, array,
		...
		try {
			String[] array = new String[] { "I'm a string" };
			client.findUserSolutions(array, array, true, "user", array, array, null, array, new RestPageRequest(0, 1));
		} catch (ResourceAccessException ex) {
			logger.info("Client failed as expected: {}", ex.toString());
		}
		try {
