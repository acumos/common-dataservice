					...
					for (CMSWorkspace ws : CMSWorkspace.values()) {
						...
						if (cmsRevDesc == null || cmsRevDesc.getDescription() == null
							...
							try {
								...
								logger.info("Revision {} access {} description already migrated", r.getRevisionId(), ws.getCmsKey());
							} catch (HttpStatusCodeException ex) {
								logger.info("Migrating revision {} access {} description", r.getRevisionId(), ws.getCmsKey());
