...
public class CMSReaderClient {
	...
	/**
	 * GET http://acumos.org:8085/site/api-manual/????
	 * 
	 * @return List of carousel image names
	 */
	public CMSNameList getCarouselImageNames() {
		URI uri = buildUri(new String[] { API_MANUAL_PATH, SOLUTION_UC_PATH, SOLUTION_ASSETS_PATH, "BOGUSBOGUS" },
				null);
		logger.debug("getCarouselImageNames: uri {}", uri);
		ResponseEntity<CMSNameList> response = restTemplate.exchange(uri, HttpMethod.GET, null,
				new ParameterizedTypeReference<CMSNameList>() {
				});
		return response.getBody();
	...
	/**
	 * Gets carousel binary image as byte array
	 * 
	 * GET http://acumos.org:8085/site/api-manual/Solution/carouselImages/{path}
	 * 
	 * where {path} = event_carousel_bg_1, .....
	 * 
	 * @param name
	 *                 Image name
	 * 
	 * @return Bytes
	 */
	public byte[] getCarouselImage(String name) {
		URI uri = buildUri(new String[] { API_MANUAL_PATH, SOLUTION_UC_PATH, CAROUSEL_IMAGES_PATH, name }, null);
