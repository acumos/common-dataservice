...
public class FOMDomainTest {
	...
	public void testMLPSolutionRevisionFOM() {
		...
		@Test
	// Test for annotation errors in the FOM classes 
	...
	public void testFomVsSimple() throws Exception {
		...
		Assert.assertEquals(sol.size(), solPlain.size());
		...
		Assert.assertEquals(rev.size(), revPlain.size());
		...
		Assert.assertEquals(art.size(), artPlain.size());
	}
