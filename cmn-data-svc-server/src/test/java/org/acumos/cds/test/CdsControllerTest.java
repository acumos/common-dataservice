...
public class CdsControllerTest {
	...
	public void testTaskStepResult() throws Exception {
		...
		try {
			...
			MLPTaskStepResult sr = new MLPTaskStepResult(ct.getTaskId(), stepName, "SU",
		...
		try {
			client.addTaskStepResult(new MLPTaskStepResult(0L, "bog", "us", Instant.now()));
			throw new Exception("Unexpected success");
		} catch (HttpStatusCodeException ex) {
			logger.info("create step result failed on missing task as expected: {}", ex.getResponseBodyAsString());
		}
		try {
			client.addTaskStepResult(new MLPTaskStepResult(ct.getTaskId(), "name", "XY", Instant.now()));
			throw new Exception("Unexpected success");
		} catch (HttpStatusCodeException ex) {
