...
public class CdsControllerTest {
	...
	public void testWorkbenchArtifacts() throws Exception {
		...
		try {
			...
			cpr = new MLPProject(projName, cu.getUserId());
			...
			cnb = new MLPNotebook("nb name 1", cu.getUserId());
			...
			cpl = new MLPPipeline("pl name 1", cu.getUserId());
		...
		try {
			MLPProject bogusUpdate = new MLPProject(cpr);
			bogusUpdate.setProjectId("999L");
			client.updateProject(bogusUpdate);
			throw new Exception("Unexpected success");
		} catch (HttpStatusCodeException ex) {
			logger.info("update failed on bad id as expected: {}", ex.getResponseBodyAsString());
		}
		...
		try {
			MLPNotebook bogusUpdate = new MLPNotebook(cnb);
			bogusUpdate.setNotebookId("999L");
			client.updateNotebook(bogusUpdate);
			throw new Exception("Unexpected success");
		} catch (HttpStatusCodeException ex) {
			logger.info("update failed on bad id as expected: {}", ex.getResponseBodyAsString());
		}
