...
public class CdsControllerTest {
	...
	public void createSolutionWithArtifacts() throws Exception {
		...
		try {
			...
			logger.info("Querying for private user solutions via flexible i/f");
			RestPageResponse<MLPSolution> userPrivateSols = client.findUserPrivateSolutions(null, null, true,
					inactiveUser.getUserId(), null, null, null, new RestPageRequest(0, 5));
			Assert.assertTrue(userPrivateSols != null && userPrivateSols.getNumberOfElements() > 0);
