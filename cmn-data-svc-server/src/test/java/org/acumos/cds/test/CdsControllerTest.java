...
public class CdsControllerTest {
	...
	public void testTaskStepResult() throws Exception {
		...
		try {
			...
			MLPStepResult sr = new MLPStepResult(ct.getTaskId(), "OB", stepName, "SU", Instant.now().minusSeconds(60));
			...
			MLPStepResult getById = client.getTaskStepResult(sr.getTaskId(), sr.getStepResultId());
			Assert.assertNotNull(getById.getStepResultId());

			List<MLPStepResult> stepResults = client.getTaskStepResults(sr.getTaskId());
			...
			RestPageResponse<MLPStepResult> emptySearchResults = client.searchTaskStepResults(queryParameters, true,
					new RestPageRequest(0, 10));
			Assert.assertEquals(0, emptySearchResults.getNumberOfElements());

			queryParameters.put("name", stepName);
			RestPageResponse<MLPStepResult> searchResults = client.searchTaskStepResults(queryParameters, true,
					new RestPageRequest(0, 10));
		...
		try {
			client.addTaskStepResult(new MLPStepResult(ct.getTaskId(), "bogus", "name", "FA", Instant.now()));
			throw new Exception("Unexpected success");
		} catch (HttpStatusCodeException ex) {
			logger.info("create step result failed on bad type code as expected: {}", ex.getResponseBodyAsString());
		}
		try {
			MLPStepResult stepResult = new MLPStepResult(0L, "bo", "gu", "s", Instant.now());
