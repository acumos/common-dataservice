...
public class CdsControllerTest {
	...
	public void testThreadsComments() throws Exception {
		...
		long commentCount = client.getCommentCount(thread.getThreadId());
		Assert.assertTrue(commentCount > 0);

		ThreadCommentsTransport threadComments = client.getThreadComments(thread.getThreadId());
		Assert.assertTrue(threadComments != null && !threadComments.getComments().isEmpty());
