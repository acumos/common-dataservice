	...
	public void testRepositories() throws Exception {
		...
		try {
			...
			Page<MLPSolution> solsByDate = solutionSearchService.findSolutionsByModifiedDate(true, accTypes,
					modifiedTs, new PageRequest(0, 5, null));
			...
			// When the columns were modeled as Date this test required Thread.sleep(1000)
			// invocations, I now believe because the driver only passed seconds to Mariadb.
			
			// TODO Zero/tiny time difference works on derby (code and db in same Java VM)
			// TODO but not when talking to a separate DB server on the same host. 
			// TODO Seems that Derby driver for TIMESTAMP has high precision but Mariadb 
			// TODO driver for TIMESTAMP has low precision?
			Timestamp beforeUpdateTs = new Timestamp(new Date().getTime());
