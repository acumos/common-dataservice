...
public class CdsRepositoryServiceTest {
	...
	public void testPeerSolutionGroups() {
		...
		MLPPeerGrpMemMap gpm = new MLPPeerGrpMemMap(pg1.getGroupId(), pr.getPeerId());
		gpm = peerGroupMemMapRepository.save(gpm);
		logger.info("Created peer group map " + gpm);
		Page<MLPPeer> peers = peerGroupMemMapRepository.findPeersByGroupId(pg1.getGroupId(), pageable);
		...
		Assert.assertNotNull(accessPeers);
		...
		peerGroupMemMapRepository.delete(gpm);
