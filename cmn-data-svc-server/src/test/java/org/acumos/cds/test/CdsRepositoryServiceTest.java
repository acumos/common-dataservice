...
public class CdsRepositoryServiceTest {
	...
	public void testTaskStepResults() throws Exception {
		try {
			...
			Page<MLPTaskStepResult> page = stepResultSearchService.findStepResults(null, null, null, null, null,
					statusCode, false, PageRequest.of(0, 5));
			Assert.assertNotEquals(0, page.getNumberOfElements());

			// Test search with empty result
			Page<MLPTaskStepResult> emptySteps = stepResultSearchService.findStepResults(0L, "bogus", "bogus", "bogus",
					"bogus", "bogus", false, PageRequest.of(0, 5));
			Assert.assertTrue(emptySteps.isEmpty());
	...
	public void testErrorConditions() throws Exception {
		...
		try {
			stepResultSearchService.findStepResults(null, null, null, null, null, null, false, PageRequest.of(0, 5));
			throw new Exception("Unexpected success");
		} catch (IllegalArgumentException ex) {
			logger.info("Search failed on missing query as expected: {}", ex.toString());
		}
