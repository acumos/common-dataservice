...
public class CdsRepositoryServiceTest {
	...
	public void testRepositories() throws Exception {
		...
		try {
			...
			Page<MLPUser> userPage = userService.findUsers(restr, true, PageRequest.of(0, 5));
			...
			Page<MLPPeer> searchPeers = peerSearchService.findPeers(peerParms, false, PageRequest.of(0, 5));
			...
			Page<MLPRole> searchRoles = roleSearchService.findRoles(roleParms, false, PageRequest.of(0, 5));
			...
			Page<MLPArtifact> searchArts = artifactSearchService.findArtifacts(artParms, false, PageRequest.of(0, 5));
			...
			Page<MLPSolution> searchSols = solutionSearchService.findSolutions(solParms, false, PageRequest.of(0, 1));
			...
			Page<MLPSolution> oneTagSearchResult = solutionSearchService.findPortalSolutions(null, null, active, null,
					null, null, null, searchTags, null, null, PageRequest.of(0, 5));
			...
			Page<MLPSolution> solsByDate = solutionSearchService.findSolutionsByModifiedDate(true, accTypes, valCodes,
					modifiedDate, PageRequest.of(0, 5));
			...
			Page<MLPSolution> recentlyUpdated = solutionSearchService.findSolutionsByModifiedDate(true, accTypes,
					valCodes, justAboutNow, PageRequest.of(0, 5));
			...
			Page<MLPSolution> solByCriteria = solutionSearchService.findSolutionsByModifiedDate(true, accessTypes,
					valStatuses, anHourAgo, PageRequest.of(0, 5));
	...
	public void createSolutionWithArtifacts() throws Exception {
		...
		try {
			...
			restr.put("firstName", firstName);
			restr.put("lastName", lastName);
			Page<MLPUser> userPage = userService.findUsers(restr, false, PageRequest.of(0, 5));
			...
			Optional<MLPSolutionWeb> optStats = solutionWebRepository.findById(stats.getSolutionId());
			Assert.assertTrue(optStats.isPresent());
			final Long countAfter = optStats.get().getViewCount();
			...
			if (cleanup) {
				logger.info("Removing newly added entities");
				MLPSolTagMap.SolTagMapPK solTagMapKey = new MLPSolTagMap.SolTagMapPK(cs.getSolutionId(), tag1.getTag());
				solutionWebRepository.deleteById(cs.getSolutionId());
