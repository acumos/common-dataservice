...
public class CdsRepositoryServiceTest {
	...
	public void testWorkbenchArtifacts() throws Exception {
		try {
			...
			MLPProject cpr = new MLPProject("proj name", cu.getUserId());
			cpr = projectRepository.save(cpr);
			Assert.assertNotNull(cpr.getProjectId());

			MLPNotebook cnb = new MLPNotebook("nb name", cu.getUserId());
			cnb = notebookRepository.save(cnb);
			Assert.assertNotNull(cnb.getNotebookId());

			MLPPipeline cpl = new MLPPipeline("pl name", cu.getUserId());
			...
			Iterable<MLPNotebook> nbs = projNbMapRepository.findNotebooksByProjectId(cpr.getProjectId());
			Assert.assertTrue(nbs.iterator().hasNext());
			Iterable<MLPPipeline> pls = projPlMapRepository.findPipelinesByProjectId(cpr.getProjectId());
