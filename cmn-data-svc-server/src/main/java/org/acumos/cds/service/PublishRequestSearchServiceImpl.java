...
/**
 * Hibernate-assisted methods to search publish request information.
 */
...
public class PublishRequestSearchServiceImpl extends AbstractSearchServiceImpl implements PublishRequestSearchService {
	...
	@Override
	public Page<MLPPublishRequest> findPublishRequests(Map<String, Object> queryParameters, boolean isOr, Pageable pageable) {

		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(MLPPublishRequest.class);
		...
		List<MLPPublishRequest> items = criteria.list();
		logger.info("findPublishRequests: result size={}", items.size());
