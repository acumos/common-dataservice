...
public class RightToUseSearchServiceImpl extends AbstractSearchServiceImpl implements RightToUseSearchService {
	 ...
	 * @return Predicate
	 */
	private Predicate createRtuPredicate(Root<MLPRightToUse> from, String lumId, String solutionId, Boolean site,
			boolean isOr) {

		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		List<Predicate> predicates = new ArrayList<>();
		if (lumId != null && !lumId.isEmpty())
			predicates.add(cb.equal(cb.lower(from.<String>get(MLPRightToUse_.lumId)), lumId.toLowerCase()));
	 ...
	 */
	@Override
	public Page<MLPRightToUse> findRtus(String lumId, String solutionId, Boolean site, boolean isOr,
			Pageable pageable) {
		...
		countQueryDef.where(createRtuPredicate(countFrom, lumId, solutionId, site, isOr));
		...
		rootQueryDef.where(createRtuPredicate(countFrom, lumId, solutionId, site, isOr));
