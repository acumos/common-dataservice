...
import org.acumos.cds.util.LoggingConstants;
...
public class LoggingHandlerInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		addKey(LoggingConstants.MDC_SERVER_IP_ADDRESS, InetAddress.getLocalHost().getHostAddress());
		addKey(LoggingConstants.MDC_REMOTE_HOST, request.getRemoteAddr());
		final String requestId = request.getHeader("X-Request-ID");
		if (requestId != null)
			addKey(LoggingConstants.MDC_KEY_REQUEST_ID, requestId);
		return true;
	}
