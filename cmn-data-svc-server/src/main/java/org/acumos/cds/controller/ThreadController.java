...
public class ThreadController extends AbstractController {
	...
	/**
	 * Returns a list of comments sorted by created date-time.
	 * 
	 * @param threadId
	 *            Thread ID
	 * @param response
	 ...
	 */
	@ApiOperation(value = "Gets all comments in the thread.", response = MLPComment.class,  responseContainer = "List")
	...
	public Object getThreadComments(@PathVariable("threadId") String threadId, HttpServletResponse response) {
		...
		return commentRepository.findByThreadIdOrderByCreated(threadId);
	}
