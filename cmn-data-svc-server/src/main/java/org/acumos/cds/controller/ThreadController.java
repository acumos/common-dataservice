...
public class ThreadController extends AbstractController {
	...
	public Object updateThread(@PathVariable("threadId") String threadId, @RequestBody MLPThread thread,
		...
		if (existing == null) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return new ErrorTransport(HttpServletResponse.SC_BAD_REQUEST, "Failed to find thread " + threadId,
					null);
		}
	 ...
	 * within the parent.  Database sorts them by created date-time.
	 * 
	 * @param threadId
	 *            Thread ID
	 * @param response
	 ...
	 */
	@ApiOperation(value = "Gets all comments in the thread.", response = ThreadCommentsTransport.class)
	@RequestMapping(value = "{threadId}/" + CCDSConstants.COMMENT_PATH, method = RequestMethod.GET)
	@ResponseBody
	public Object getThreadComments(@PathVariable("threadId") String threadId, HttpServletResponse response) {
		MLPThread thread = threadRepository.findOne(threadId);
		if (thread == null) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return new ErrorTransport(HttpServletResponse.SC_BAD_REQUEST, "Failed to find thread " + threadId,
		...
		List<MLPComment> comments = commentRepository.findByThreadIdOrderByCreated(threadId);
		ThreadCommentsTransport result = new ThreadCommentsTransport((long)comments.size(), thread.getTitle(), thread.getUrl());
