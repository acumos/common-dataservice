...
public class GroupPeerSolutionController extends AbstractController {
	...
	@ResponseBody
	public Page<MLPSolution> getSolutionsInGroup(@PathVariable("groupId") Long groupId, Pageable pageRequest) {
	...
	public Object dropSolutionFromGroup(@PathVariable("groupId") Long groupId,
		...
		if (solGroupMemMapRepository.findOne(pk) == null) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return new ErrorTransport(HttpServletResponse.SC_BAD_REQUEST, NO_MAP_WITH_IDS + groupId, null);
		}
	...
	public Object unmapPeerSolutionGroups(@PathVariable("peerGroupId") Long peerGroupId,
		...
		if (peerSolAccMapRepository.findOne(pk) == null) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
	...
	public Object unmapPeerPeerGroups(@PathVariable("principalGroupId") Long principalGroupId,
		...
		MLPPeerPeerAccMap.PeerPeerAccMapPK pk = new MLPPeerPeerAccMap.PeerPeerAccMapPK(principalGroupId,
				resourceGroupId);
		if (peerPeerAccMapRepository.findOne(pk) == null) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return new ErrorTransport(HttpServletResponse.SC_BAD_REQUEST,
					NO_MAP_WITH_IDS + principalGroupId + "," + resourceGroupId, null);
		}
