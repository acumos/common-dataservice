...
import org.acumos.cds.domain.MLPProjNbMap;
import org.acumos.cds.domain.MLPProjPlMap;
import org.acumos.cds.domain.MLPProject;
import org.acumos.cds.repository.NotebookRepository;
import org.acumos.cds.repository.PipelineRepository;
import org.acumos.cds.repository.ProjNbMapRepository;
import org.acumos.cds.repository.ProjPlMapRepository;
...
public class WorkbenchController extends AbstractController {
	...
	@Autowired
	private ProjNbMapRepository projNbMapRepository;
	@Autowired
	private ProjPlMapRepository projPlMapRepository;
	...
	@ApiOperation(value = "Adds the specified notebook to the specified project.", //
			response = SuccessTransport.class)
	...
	public MLPResponse addNotebookToProject(@PathVariable("projectId") String projectId,
		...
		projNbMapRepository.save(new MLPProjNbMap(projectId, notebookId));
		return new SuccessTransport(HttpServletResponse.SC_OK, null);
	}

	@ApiOperation(value = "Drops the specified notebook from the specified project.", //
			response = SuccessTransport.class)
	@ApiResponses({ @ApiResponse(code = 400, message = "Bad request", response = ErrorTransport.class) })
	@RequestMapping(value = CCDSConstants.PROJECT_PATH + "/{projectId}" + CCDSConstants.NOTEBOOK_PATH
	...
	public MLPResponse dropNotebookFromProject(@PathVariable("projectId") String projectId,
		...
		try {
			MLPProjNbMap.ProjNbMapPK pk = new MLPProjNbMap.ProjNbMapPK(projectId, notebookId);
			projNbMapRepository.deleteById(pk);
			return new SuccessTransport(HttpServletResponse.SC_OK, null);
		} catch (Exception ex) {
			// e.g., EmptyResultDataAccessException is NOT an internal server error
	...
	@ApiOperation(value = "Adds the specified pipeline to the specified project.", //
			response = SuccessTransport.class)
	...
	public MLPResponse addPipelineToProject(@PathVariable("projectId") String projectId,
		...
		projPlMapRepository.save(new MLPProjPlMap(projectId, pipelineId));
		return new SuccessTransport(HttpServletResponse.SC_OK, null);
	}

	@ApiOperation(value = "Drops the specified pipeline from the specified project.", //
			response = SuccessTransport.class)
	@ApiResponses({ @ApiResponse(code = 400, message = "Bad request", response = ErrorTransport.class) })
	@RequestMapping(value = CCDSConstants.PROJECT_PATH + "/{projectId}" + CCDSConstants.PIPELINE_PATH
	...
	public MLPResponse dropPipelineFromProject(@PathVariable("projectId") String projectId,
		...
		try {
			MLPProjPlMap.ProjPlMapPK pk = new MLPProjPlMap.ProjPlMapPK(projectId, pipelineId);
			projPlMapRepository.deleteById(pk);
			return new SuccessTransport(HttpServletResponse.SC_OK, null);
		} catch (Exception ex) {
			// e.g., EmptyResultDataAccessException is NOT an internal server error
