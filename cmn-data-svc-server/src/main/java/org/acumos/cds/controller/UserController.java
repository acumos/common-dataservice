...
public class UserController extends AbstractController {
	...
	public CountTransport getUserCount() {
		...
		logger.audit(beginDate, "getUserCount {}", count);
	...
	public Object addOrDropUsersInRole(@PathVariable("roleId") String roleId,
		...
		// Done
		logger.audit(beginDate, "addOrDropUsersInRole: role {}", roleId);
		return new SuccessTransport(HttpServletResponse.SC_OK, null);
	}
	...
	public Object createUserNotificationPreference(@RequestBody MLPUserNotifPref usrNotifPref,
		...
		try {
			...
			logger.audit(beginDate, "createUserNotificationPreference: userId {}", usrNotifPref.getUserId());
			return result;
		} catch (Exception ex) {
			Exception cve = findConstraintViolationException(ex);
