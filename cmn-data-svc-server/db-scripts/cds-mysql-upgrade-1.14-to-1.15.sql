...
-- Also notice that if the select query on C_USER retruns more than 1 record,  
-- then update the emails of all those records to be distinct non null strings so 
-- that addition of non null constraint on the unique EMAIL field during upgrade succeeds.    
-- 0
...
-- 11
ALTER TABLE C_USER MODIFY EMAIL VARCHAR(100) NOT NULL;